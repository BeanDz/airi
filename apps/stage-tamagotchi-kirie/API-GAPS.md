...
0.4.2. GAP-019, GAP-026, and GAP-027 have additional evidence from 2026-09-18.
...
- Proposed upstream fix: [godot-kirie#90](https://github.com/moeru-ai/godot-kirie/pull/90) exports the `backRequested` event contract from `@gd-kirie/platform`. `GdKiriePlatformHost` connects to `Window.GoBackRequested` and re-emits `kirie:platform:back:requested`. The decisions are recorded as ADR-0005 and ADR-0006 in that repository.
- Reopen condition: After a Kirie release includes the capability, subscribe through `context.on(backRequested, ...)`, delete the custom back event, and remove `MobileNavigationService.RequestBack` and the `Main.cs` notification bridge.
