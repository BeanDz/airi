...
- Remaining fail-fast concern: Kirie 0.4.1 still drops a registered invoke when the context has no handler. [godot-kirie#87](https://github.com/moeru-ai/godot-kirie/pull/87) rejects that invoke with a `-receive-error` response. AIRI cannot consume this change until a Kirie release after 0.4.1. This ledger tracks the AIRI window-binding fix only.
