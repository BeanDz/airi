...
0.5.0. GAP-019, GAP-026, and GAP-027 have additional evidence from 2026-09-18.
...
full chat send round trip in real CEF. User review accepted GAP-029 on
...
GAP-030 was recorded on 2026-09-22 for the Android system Back request, and
Kirie Platform 0.5.0 ships the capability that replaces the AIRI host bridge.
...
- `In progress`: Implementation exists, but required behavior is not complete.
...
| GAP-030 | Android main renderer | Press the Android system Back button | Electron owns the application window and maps Back to window close or history | Deliver the system Back request to the main renderer so the page decides whether to navigate or quit | Kirie Platform and AIRI Godot host | Runtime verification pending | Recorded 2026-09-22; Kirie Platform 0.5.0 ships `backRequested` and AIRI subscribes through `context.on(backRequested, ...)`; the custom back event and `MobileNavigationService.RequestBack` are removed |
...
- Existing Kirie capabilities: `PlatformClient.globalShortcuts.register`, `unregister`, `notifications.show`, and `notifications.onActivated`.
- Required result: AIRI stores the Spotlight shortcut, opens one reusable native Spotlight window when the shortcut fires, hides that window without destroying it, and shows a result notification that can open Chat.
- Ownership: This is AIRI window orchestration. The Godot host owns the window, shortcut persistence, and Eventa contracts. The main renderer registers the OS shortcut through Kirie Platform outside the GAP-008 map. The Spotlight renderer shows notifications through Kirie Platform. This does not add a Kirie Core API.
- Contract result: Settings and the main renderer bind `spotlight:shortcut:get` and `spotlight:shortcut:set`. The Spotlight renderer binds `spotlight:hide`, microphone permission, and `chat:open`. The main renderer binds `spotlight:open`.
- Window result: The host creates one 720 x 100 always-on-top borderless window at `#/spotlight` with the minimal follower runtime. Close and blur hide the window. The next shortcut reuses it.
...
- Dependency baseline: AIRI now uses the coordinated Kirie 0.4.2 npm and NuGet packages.
...
- Dependency result: Kirie 0.4.2 exposes `PermissionRequested`, `GrantPermission()`, and `DenyPermission()`. The AIRI addon matches the published Kirie 0.4.2 addon.
...
- Previous runtime result: A real Controls Island sign-in completed on 2026-09-17. The hosted callback page reported that AIRI received the sign-in result.
...
- Review result: The OIDC implementation was approved on 2026-09-17 with no blocking findings.
- Shared-context result: Sign-in with the shared-context build updated the account page with the authenticated state on 2026-09-18.
- Acceptance result: The user verified the complete sign-in and account flow with the running AIRI application.
...
- Original Inspector result: **Open** launched `http://127.0.0.1:9229/`. Godot CEF returned `200` with an empty response body.
...
- Upstream fix: Kirie Platform 0.5.0 exports the `backRequested` contract from `@gd-kirie/platform`. `GdKiriePlatformHost` connects to `Window.GoBackRequested` and re-emits `kirie:platform:back:requested`. The decisions are recorded as ADR-0005 and ADR-0006 in `moeru-ai/godot-kirie`.
- Current AIRI implementation: `Main.cs` keeps `SceneTree.QuitOnGoBack` disabled. `installMobileNavigation` subscribes through `context.on(backRequested, ...)` and applies the route or quit policy. The custom `eventa:event:airi:mobile:back-requested` event and `MobileNavigationService.RequestBack` are removed.
- Remaining work: Verify the Android system Back flow on a device or emulator.
